"""
TalkNet-ASD single video active speaker detection.

Usage:
    cd speaker_detection/TalkNet-ASD
    python scripts/run.py --video_path /path/to/video.mp4
    python scripts/run.py --video_path /path/to/video.mp4 --device mps
...
Pipeline:
    ...
    3. 人脸检测 (S3FD)
    4. 人脸追踪 + 裁剪人脸片段
    5. TalkNet 模型推理 (cross-attention ASD) → 每个 track 得到逐帧 speaking score
    6. 输出可视化视频 (绿框=speaking, 红框=not speaking)
...
def evaluate_network(files, args):
    """TalkNet ASD inference on cropped face clips."""
    s = talkNet()
    s.loadParameters(args.pretrain_model)
    s.to(args.device)
    print(f"[TalkNet] Model loaded from {args.pretrain_model} on {args.device}")
    s.eval()
    allScores = []
...
def main():
    ...
    # Setup args namespace (compatible with demoTalkNet.py functions)
    ...
    # Auto-download pretrain model if not exists
    if not os.path.isfile(args.pretrain_model):
        print(f"[TalkNet] Downloading pretrained model...")
    ...
    if pargs.save_dir:
        ...
        args.savePath = os.path.join(os.path.dirname(args.videoPath), video_name + '_talknet_asd')
    ...
    print(f"[6/6] Active Speaker Detection (TalkNet)...")
    ...
    # Print summary
    print(f"\n{'='*50}")
    print(f"Speaker Detection Summary (TalkNet-ASD):")
